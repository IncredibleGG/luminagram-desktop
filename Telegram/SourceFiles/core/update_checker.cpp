...
// LuminaGram: MtpChecker is started empty (single HTTP feed), so this
// canary-session lookup has no caller; kept for upstream parity.
[[maybe_unused]] [[nodiscard]] base::weak_ptr<Main::Session> LookupCanaryPrivateSession(
...
[[nodiscard]] bool WriteUpdateReadyFile(const QString &readyFilePath) {
	// LuminaGram: write our marker instead of stock "1" so checkReadyUpdate()
	// refuses to install packages this fork did not produce (see kReadyMarker).
...
bool checkReadyUpdate() {
	...
	// The bundle inside an update package carries our own name, not Telegram's -
	// the line above already resolves it that way for the installed copy.
	QFileInfo updater(cWorkingDir()
		+ u"tupdates/temp/"_q
		+ cExeName()
